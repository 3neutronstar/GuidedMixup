...
class DenseNet_ft(DenseNet):
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=64, bn_size=4, drop_rate=0, num_classes=1000, memory_efficient=False):
        super().__init__(growth_rate=growth_rate, block_config=block_config, num_init_features=num_init_features, bn_size=bn_size, drop_rate=drop_rate, num_classes=num_classes, memory_efficient=memory_efficient)
...
def _densenet(arch, growth_rate, block_config, num_init_features, pretrained, progress,train_mode='baseline',
              **kwargs):
    if train_mode in ['snapmix','cam_saliency','spmmixup']:
        model = DenseNet_ft(growth_rate, block_config, num_init_features, **kwargs)
    else:
...
def densenet(configs):
    ...
    if 'pretrained' in configs['model']:
        ...
        pretrained=False
        model=dense_dict[configs['model']](pretrained,num_classes=configs['num_classes'],train_mode=configs['train_mode'])
    return model
